...
new String[]{"Metric", "Value"})); 
        tips.setText(String.format("📊 Your Eco Progress:%n%n• %d emails tracked%n• %.3f kg total CO₂ emissions%n• %.3f kg saved with eco options%n• Current eco score: %.0f%%%n%n💡 Quick Tips:%n• Compress attachments before sending%n• Use eco-friendly email providers%n• Enable green server options%n• Batch similar messages together", emails, co2, saved, score)); 
    } 
...
title.setForeground(G.darker()); 
        ...
        JPanel nav = new JPanel(new FlowLayout(FlowLayout.RIGHT)); 
nav.setBackground(B); 
        Btn backBtn = new Btn("⬅️ Back to Dashboard", G, GL); 
...
45)); 
        String[] acts = {"👥 Manage Users", "📊 System Analytics", "⚙️ Configuration", "📈 Generate Reports", "💾 Backup Database", "🔄 Reset Settings"}; 
